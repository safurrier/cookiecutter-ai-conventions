...
def parse_learnings(staging_path: Path) -> List[Dict[str, str]]:
    ...
    learning_blocks = re.split(r"^## \d{4}-\d{2}-\d{2}:", content, flags=re.MULTILINE)[
        1:
    ]
    ...
    for block in learning_blocks:
        ...
        learning = {
            ...
            "date": re.search(r"(\d{4}-\d{2}-\d{2})", block).group(1)
...
def suggest_promotions(
    ...
    for _domain, domain_learnings in by_domain.items():
        for learning in domain_learnings:
            ...
            if (
                days_old >= 7
                and learning.get("problem")
                and learning.get("solution")
                and learning.get("promote_to")
            ):
...
def display_review(
    learnings: List[Dict[str, str]],
    by_domain: Dict[str, List[Dict[str, str]]],
    suggestions: List[Tuple[Dict[str, str], str]],
) -> None:
...
def archive_old_learnings(learnings: List[Dict[str, str]], staging_path: Path) -> None:
    ...
    old_learnings = [
        learning
        for learning in learnings
        if calculate_days_old(learning.get("date", "")) > 30
    ]
    ...
    if old_learnings:
        ...
        if archive == "y":
            archive_path = (
                staging_path.parent
                / "archive"
                / f"learnings_{datetime.now().strftime('%Y%m')}.md"
            )
